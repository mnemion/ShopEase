...
class OAuth2CallbackView(OAuth2View):
    def dispatch(self, request, *args, **kwargs):
        # ========= 디버깅 로그 추가 시작 =========
        logger.debug("="*30)
        logger.debug("[OAuth2CallbackView dispatch] 시작")
        logger.debug(f"[OAuth2CallbackView dispatch] Request Method: {request.method}")
        logger.debug(f"[OAuth2CallbackView dispatch] Request GET params: {request.GET}")
        # ========= 디버깅 로그 추가 끝 ===========

        if "error" in request.GET or "code" not in request.GET:
            # TODO: Handle error response gracefully.
            logger.error(f"[OAuth2CallbackView dispatch] 콜백 요청에 에러 또는 코드 없음: {request.GET}")
            return render_authentication_error(
                request,
                provider=self.adapter.provider_id,
                exception=request.GET.get("error"),
            )
        ...
        try:
            # ========= 디버깅 로그 추가 시작 =========
            logger.debug(f"[OAuth2CallbackView dispatch] 액세스 토큰 가져오기 시도...")
            # ========= 디버깅 로그 추가 끝 ===========
            access_token = client.get_access_token(request.GET["code"])
            token = self.adapter.parse_token(access_token)
            # ========= 디버깅 로그 추가 시작 =========
            logger.debug(f"[OAuth2CallbackView dispatch] 액세스 토큰 수신 성공: {access_token}")
            logger.debug(f"[OAuth2CallbackView dispatch] 파싱된 토큰: {token}")
            # ========= 디버깅 로그 추가 끝 ===========
            token.app = app
            ...
            if self.adapter.supports_state:
                login.state = SocialLogin.verify_and_unstash_state(
                    request, self.adapter.get_state_from_request(request)
                )
            # ========= 디버깅 로그 추가 시작 =========
            logger.debug(f"[OAuth2CallbackView dispatch] complete_login 호출 완료. 결과: {login}")
            # ========= 디버깅 로그 추가 끝 ===========
            final_response = complete_social_login(request, login)
            logger.debug(f"[OAuth2CallbackView dispatch] complete_social_login 반환 결과 (응답): {final_response}")
            if hasattr(final_response, 'status_code'):
                logger.debug(f"[OAuth2CallbackView dispatch] 응답 상태 코드: {final_response.status_code}")
                if final_response.status_code == 302:
                    logger.debug(f"[OAuth2CallbackView dispatch] 리디렉션 URL: {final_response.url}")
            ...
            logger.error(f"[OAuth2CallbackView dispatch] 처리 중 예외 발생: {type(e).__name__} - {str(e)}", exc_info=True)
            return render_authentication_error(
